...
def deepnn(example_input, keep_prob):
    def conv2d(x, W):
    ...
    # "features" - there is only one here, since images are grayscale -- it
    # would be 3 for an RGB image, 4 for RGBA, etc.
    ...
    # features.
    # activation_fc1_drop = tf.nn.dropout(activation_fc1, keep_prob)
    ...
    # return tf.matmul(activation_fc1_drop, thetas_fc2) + theta0_fc2
...
if __name__ == '__main__':
    ...
    model_output = deepnn(example_input, keep_prob)
        ...
        for i in range(20000):
          ...
          if i % 100 == 0:
            train_accuracy = accuracy.eval(feed_dict={
                example_input: batch[0], example_output: batch[1], keep_prob: 1.0})
            print('step %d, training accuracy %g' % (i, train_accuracy))
          train_step.run(feed_dict={example_input: batch[0], example_output: batch[1], keep_prob: 0.5})

        print('test accuracy %g' % accuracy.eval(feed_dict={
            example_input: mnist.test.images, example_output: mnist.test.labels, keep_prob: 1.0}))
